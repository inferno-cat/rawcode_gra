...
class SamePaddingConv2d(nn.Module):
    ...
    def check_spatial_shape(self, x):
        if x.size(2) != self._in_spatial_shape[0] or x.size(3) != self._in_spatial_shape[1]:
            raise ValueError(
...
class MBConvBlockV2(nn.Module):
    def __init__(
        ...
        if expansion_factor != 1:
            self.expand_conv = nn.Conv2d(in_channels=in_channels, out_channels=exp_channels, kernel_size=1, bias=bias)

            self.expand_bn = nn.BatchNorm2d(num_features=exp_channels, eps=bn_epsilon, momentum=bn_momentum)
        ...
        self.dp_bn = nn.BatchNorm2d(num_features=exp_channels, eps=bn_epsilon, momentum=bn_momentum)
        ...
        if se_size is not None:
            self.se = SqueezeExcitate(exp_channels, se_size, activation=get_activation(act_fn, **act_kwargs))
            self.ops_lst.append(self.se)

        # projection layer
        self.project_conv = nn.Conv2d(in_channels=exp_channels, out_channels=out_channels, kernel_size=1, bias=bias)

        self.project_bn = nn.BatchNorm2d(num_features=out_channels, eps=bn_epsilon, momentum=bn_momentum)
...
class EfficientNetV2(nn.Module):
    ...
    def __init__(
        ...
        for stage_args in stages:
            ...
            in_channels = round_filters(in_channels, self.cfg["width_coefficient"])
            out_channels = round_filters(out_channels, self.cfg["width_coefficient"])
        ...
        self.head_conv = nn.Conv2d(
            in_channels=in_channels, out_channels=head_conv_out_channels, kernel_size=1, bias=bias
        )
        self.head_bn = nn.BatchNorm2d(num_features=head_conv_out_channels, eps=bn_epsilon, momentum=bn_momentum)
    ...
    def _load_state(self, in_channels, n_classes, progress, tf_style_conv):
        state_dict = model_zoo.load_url(self.cfg["weight_url"], progress=progress, file_name=self.cfg["model_name"])
...
if __name__ == "__main__":
    ...
    model = EfficientNetV2(model_name="l", in_channels=3)
    x = torch.randn(1, 3, 480, 320)
    y = model.get_features(x)
    for i in range(len(y)):
        print(y[i].shape)
    param = get_model_parm_nums(model)
    print(param)
