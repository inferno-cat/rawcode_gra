...
class SamePaddingConv2d(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        pad_along_height = max(
            (out_height - 1) * stride_heigth + filter_height + (filter_height - 1) * (dilation_height - 1) - in_height,
            0,
        )
        pad_along_width = max(
            (out_width - 1) * stride_width + filter_width + (filter_width - 1) * (dilation_width - 1) - in_width, 0
        )
    ...
    def check_spatial_shape(self, x):
        if x.size(2) != self.in_spatial_shape[0] or x.size(3) != self.in_spatial_shape[1]:
            raise ValueError(
                "Expected input spatial shape {}, got {} instead".format(self.in_spatial_shape, x.shape[2:])
            )
...
class ConvBNAct(nn.Module):
    def __init__(self, out_channels, activation=None, bn_epsilon=None, bn_momentum=None, same_padding=False, **kwargs):
...
class Swish(nn.Module):
    def __init__(self, beta=1.0, beta_learnable=False):
        super(Swish, self).__init__()

        if beta == 1.0 and not beta_learnable:
            self._op = self.simple_swish
        else:
            self.beta = nn.Parameter(torch.full([1], beta), requires_grad=beta_learnable)
...
class DropConnect(nn.Module):
    ...
    def forward(self, x):
        if self.training:
            random_tensor = self.keep_prob + torch.rand([x.size(0), 1, 1, 1], dtype=x.dtype, device=x.device)
...
class SqueezeExcitate(nn.Module):
    def __init__(self, in_channels, se_size, activation=None):
        ...
        self.dim_reduce = nn.Conv2d(in_channels=in_channels, out_channels=se_size, kernel_size=1)
        self.dim_restore = nn.Conv2d(in_channels=se_size, out_channels=in_channels, kernel_size=1)
...
class MBConvBlock(nn.Module):
    def __init__(
        ...
        if se_size is not None:
            self.se = SqueezeExcitate(exp_channels, se_size, activation=self.activation)
        else:
            self.se = None
...
class EnetStage(nn.Module):
    def __init__(
        self, num_layers, in_spatial_shape, in_channels, out_channels, stride, se_ratio, drop_connect_rates, **kwargs
    ):
        ...
        if not (isinstance(drop_connect_rates, container_abcs.Iterable) and len(drop_connect_rates) == num_layers):
            raise ValueError(
                "drop_connect_rates must be iterable of "
...
class EfficientNet(nn.Module):
    ...
    def __init__(
        ...
        EfficientNet.check_init_params(
            b,
            in_channels,
            n_classes,
            in_spatial_shape,
        ...
        self.head_conv = ConvBNAct(
            in_channels=head_conv_in_channels,
    ...
    def get_dc_rates(self):
        ...
        return [self.drop_connect_rate * i / total_mbconv_layers for i in range(total_mbconv_layers)]

    def _load_state(self, b, in_channels, n_classes, progress):
        state_dict = model_zoo.load_url(
            EfficientNet.state_dict_urls[b], progress=progress, file_name=EfficientNet.dict_names[b]
        )
        strict = True
        if in_channels != 3:
    ...
    def check_input(self, x):
        ...
        if x.size(1) != self.in_channels:
            raise ValueError("Input must have {} channels, got {} instead".format(self.in_channels, x.size(1)))
    ...
    def check_init_params(
        ...
        if not (
            in_spatial_shape is None
            or isinstance(in_spatial_shape, int)
            or (
                isinstance(in_spatial_shape, container_abcs.Iterable)
                and len(in_spatial_shape) == 2
                and all(isinstance(s, int) for s in in_spatial_shape)
            )
        ):
            raise ValueError(
                "in_spatial_shape must be either None, int or iterable of ints of length 2"
        ...
        if not isinstance(drop_connect_rate, float):
            ...
            raise ValueError(
                "drop_connect_rate must be within range 0 <= drop_connect_rate < 1.0, "
        ...
        if override_dropout_rate is not None:
            if not isinstance(override_dropout_rate, float):
                raise ValueError(
                    "dropout_rate must be either None or float, " "got {} instead".format(type(override_dropout_rate))
                )
            elif not 0 <= override_dropout_rate < 1.0:
                raise ValueError(
                    "dropout_rate must be within range 0 <= dropout_rate < 1.0, "
    ...
    def get_features(self, x):
        ...
        for stage in self.stages:
            ...
            if spatial_size not in feature_dict or channels > feature_dict[spatial_size][1]:
                feature_dict[spatial_size] = (x, channels)

        out = [feature for feature, _ in sorted(feature_dict.values(), key=lambda item: item[0].size(2), reverse=True)]
...
if __name__ == '__main__':
    model = EfficientNet(6, pretrained=False)
    print(model)
