...
class MixerDecoder(nn.Module):
    def __init__(self, dim):
        ...
        self.conv1x1 = nn.Sequential(
            nn.Conv2d(dim, dim, 1, 1, 0), nn.BatchNorm2d(dim), nn.ReLU(inplace=True)
        )
...
class FSRelation(nn.Module):
    ...
    def forward(self, scene_feature, features: list):
        ...
        refined_feats = [
            torch.cat([r * p, o], dim=1)
            for r, p, o in zip(relations, p_feats, features)
        ]
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        vgg16_bn = VGG(name="vgg19_bn", pretrained=True)

        self.conv1 = vgg16_bn.conv1
        self.conv2 = vgg16_bn.conv2
        self.conv3 = vgg16_bn.conv3
        self.conv4 = vgg16_bn.conv4
        self.conv5 = vgg16_bn.conv5

        self.dem5 = DetailEnhancementModule(512)
        self.dem4 = DetailEnhancementModule(512)
        self.dem3 = DetailEnhancementModule(256)
        self.dem2 = DetailEnhancementModule(128)
        ...
        self.de5 = MixerDecoder(512)
        self.de4 = MixerDecoder(512)
        self.de3 = MixerDecoder(256)
        self.de2 = MixerDecoder(128)
        self.de1 = MixerDecoder(64)

        self.up5 = upsample(2, 512, 512)
        self.up4 = upsample(2, 512, 256)
        self.up3 = upsample(2, 256, 128)
        self.up2 = upsample(2, 128, 64)

        self.compress4 = nn.Conv2d(512, 1, 1, 1, 0)
        self.compress3 = nn.Conv2d(256, 1, 1, 1, 0)
        self.compress2 = nn.Conv2d(128, 1, 1, 1, 0)
        self.compress1 = nn.Conv2d(64, 1, 1, 1, 0)

        self.output_layer = nn.Conv2d(4, 1, 1, 1, 0)
    ...
    def forward(self, x):
        ...
        dem5 = self.dem5(conv5)  # 512
        dem4 = self.dem4(conv4)  # 512
        dem3 = self.dem3(conv3)  # 256
        dem2 = self.dem2(conv2)  # 128
        dem1 = self.dem1(conv1)  # 64

        dem4_fuse = self.up5(dem5) + dem4  # 512
        dem3_fuse = self.up4(dem4_fuse) + dem3  # 256
        dem2_fuse = self.up3(dem3_fuse) + dem2  # 128
        dem1_fuse = self.up2(dem2_fuse) + dem1  # 64

        scene_embedding = self.cam(conv5)
        fsr = self.fsr(scene_embedding, [dem4_fuse, dem3_fuse, dem2_fuse, dem1_fuse])

        fsr4, fsr3, fsr2, fsr1 = fsr[0], fsr[1], fsr[2], fsr[3]
        ...
        side_output4 = F.interpolate(
            side_output4, scale_factor=8, mode="bilinear", align_corners=True
        )
        side_output3 = F.interpolate(
            side_output3, scale_factor=4, mode="bilinear", align_corners=True
        )
        side_output2 = F.interpolate(
            side_output2, scale_factor=2, mode="bilinear", align_corners=True
        )

        side_cat = torch.cat(
            [side_output4, side_output3, side_output2, side_output1], dim=1
        )
